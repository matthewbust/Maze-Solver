...
class Maze:
		...
		for attempt in range(max_attempts):
			...
			for i in range(self.size):
				for j in range(self.size):
					if random.random() < 0.25: # Wall Density : 1.0 = 100%
						maze[i, j] = 1
	...
	def get_state(self):
		# Separate channels for walls, agent, goal
		state = np.zeros((3, self.size, self.size))
		state[0] = self.maze  # Walls
		state[1, self.agent_pos[0], self.agent_pos[1]] = 1  # Agent
		state[2, self.goal_pos[0], self.goal_pos[1]] = 1  # Goal
	...
	def step(self, action):
		...
		distance_reward = (old_dist - new_dist) * 0.3  # Reward for getting closer
		
		# Small step penalty
		return self.get_state(), distance_reward - 0.05 , False, {'result': 'move'}
...
class DQNAgent:
	...
	def replay(self):
		...
		if len(self.memory) < self.batch_size * 2: # Wait for 2x batch size
			return
...
def train_agent(episodes=300, max_steps=100, maze=None):
	"""
	Train the DQN agent on a single fixed maze
	"""
	state_size = 300  # 3 channels * 10 * 10
	action_size = 4
	...
	for episode in range(episodes):
		...
		if len(win_history) >= 50:
			...
			if win_rate > best_win_rate:
				...
				if patience_counter >= patience:
					print(f"\n  No improvement for {patience} episodes. Stopping early.")
		...
		# Calculate win rate over last 20 episodes for display
		win_rate_20 = np.mean(win_history[-20:]) if len(win_history) >= 20 else 0
		
		if (episode + 1) % 50 == 0:
			avg_score = np.mean(scores[-50:])
			print(f"Episode {episode + 1}/{episodes} | "
				  f"Avg Score: {avg_score:.2f} | "
				  f"Win Rate (20): {win_rate_20:.2%} | "
	...
	if old_model_exists and not model_exists:
		...
		if choice == '1':
			try:
				...
				agent, maze, scores, win_history = train_agent(episodes=300, max_steps=100)
				print("\n--- Training Complete ---")
